...
# 테이블 생성
# 매치의 모든 플레이어의 matchRank가 0 혹은 99인 매치는 완주하지 않은 매치로 간주하고 제외
CREATE TABLE single_speed_1004_1007_fin 
SELECT T1.matchId, T2.trackId, T2.match_start_time, T2.match_end_time, T2.accountNo, T2.characterName, T2.rankinggrade2, T2.matchRank, T2.matchRetired, T2.matchWin, T2.matchTime, T2.matchtype, T2.start, T2.end
FROM
(SELECT matchId
FROM single_speed_1004_1007
WHERE matchRank = 1) AS T1
LEFT JOIN 
(SELECT * 
FROM single_speed_1004_1007) AS T2
...
# 매치 건 수 :  13,634건
SELECT COUNT(DISTINCT matchId)
FROM single_speed_1004_1007_fin;

# 유저 수 : 14,802명
SELECT COUNT(DISTINCT accountNo)
FROM single_speed_1004_1007_fin;

-- 게임 플레이 중간에 나간 유저 : 443명
SELECT COUNT(DISTINCT accountNo)
FROM single_speed_1004_1007_fin
WHERE matchRank=0;
...
# (2) 주요 지표 
# 트랙별 사용 건 수   
SELECT T2.map_name, T1.cnt_match, T1.cnt_account
FROM (SELECT trackId, COUNT(DISTINCT matchId) AS cnt_match, COUNT(accountNo) AS cnt_account
FROM single_speed_1004_1007_fin
GROUP BY 1
...
FROM single_speed_1004_1007_fin) AS TM
LEFT JOIN 
track_name AS TN
...
-- 모든 유저가 리타이어 된 경우 확인하는 쿼리 => 0건 확인 완료
-- SELECT *
...
retire_cnt AS(
		...
		FROM (
			...
			FROM single_speed_1004_1007_fin
			WHERE matchRank!=0
			GROUP BY 1)AS T1 LEFT JOIN (		
			SELECT matchId
				   ,COUNT(accountNo) AS cnt_retire_player
			FROM single_speed_1004_1007_fin
			WHERE matchRank=99
			GROUP BY 1) AS T2 ON T1.matchId = T2.matchId) AS T3
		LEFT JOIN (SELECT TM.matchId, TN.map_name
FROM
(SELECT matchId, trackId
FROM single_speed_1004_1007_fin) AS TM
LEFT JOIN 
...
-- ----------------------------------------
# test : 리타이어된 유저와 게임 종료 유저 계산 
SELECT *
FROM single_speed_1004_1007
WHERE matchId = 'f5ac000b89e8482e';